...
const Login = () => {
  ...
  const onSubmit = (data: z.infer<typeof loginSchema>) => {
    ...
    startTransition(async () => {
      try {
        const res = await login(data); // استدعاء دالة login للحصول على النتيجة
        console.log(res);

        // تحقق من أن الاستجابة تحتوي على خصائص success أو error نصية
        if (res && typeof res === 'object') {
          if ('success' in res && typeof res.success === 'string' && res.success) {
            toast.success(res.success); // فرض النوع على أنه string
            redirect("/"); // إعادة التوجيه بعد تسجيل الدخول الناجح
          } else if ('error' in res && typeof res.error === 'string' && res.error) {
            toast.error(res.error); // فرض النوع على أنه string
          } else {
